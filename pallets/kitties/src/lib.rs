...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn buy(origin: OriginFor<T>, kitty_id: KittyId) -> DispatchResult {
			...
			ensure!(KittyOnSale::<T>::contains_key(kitty_id), Error::<T>::NoOnSale);
	...
	impl<T: Config> Pallet<T> {
		fn get_next_kitty_id() -> Result<KittyId, DispatchError> {
			NextKittyId::<T>::try_mutate(|next_kitty_id| -> Result<KittyId, DispatchError> {
				let current_id = *next_kitty_id;
				*next_kitty_id = next_kitty_id.checked_add(1).ok_or::<DispatchError>(Error::<T>::InvalidKittyId.into())?;
				Ok(current_id)
			})
		}
